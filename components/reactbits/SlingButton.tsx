...
} from 'framer-motion'; // Use framer-motion since that's what's in package.json
...
const SlingButton: React.FC<SlingButtonProps> = ({
    ...
    // Call paint synchronously so the SVG string perfectly tracks the HTML button without a 1-frame delay during fast snaps
    const schedulePaint = () => {
        paint();
    };
    ...
    const handleAutoShoot = () => {
        ...
        // Ensure direction is set to pulling down
        dir.current = { ux: 0, uy: 1 };
        
        // Temporarily fake a grip so icon points correctly if needed
        grip.current = { id: -1, startX: 0, startY: 0, scale: 1, moved: true, hist: [], rawOrigin: { x: 0, y: 0 }, slop: 0 };
        
        // Animate pull down
        animY.current = animate(py, maxPull * 0.9, { 
            duration: 0.35, 
            ease: "easeOut",
            onComplete: () => {
                grip.current = null;
                ...
                settle({ x: 0, y: -4000 }); // Snap back with high velocity
                
                // Wait for the snap-back animation to start before scrolling to top
                setTimeout(() => {
                    onSend?.();
                }, 200);
            }
