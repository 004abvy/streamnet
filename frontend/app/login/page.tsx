...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    // Auto-detect when password is saved from a reset link
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      if (result.code === 'USER_NOT_FOUND') {
        // Not registered -> redirect to signup with prefilled email
        router.push(`/signup?email=${encodeURIComponent(email)}&from=login`);
      } else if (result.code === 'WRONG_PASSWORD') {
        // Registered, but entered wrong password -> stay on page & show red inline field error
