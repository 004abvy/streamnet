...
function LiveActionContent() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/live-action?page=${pageParam}&filter=${filterParam}`);
        ...
        console.warn("Failed to fetch TMDB live-action:", err);
        if (isMounted) {
          setShows([]);
          setLoading(false);
        }
  ...
  const updateQueryParams = (newFilter?: string, newPage?: number) => {
    ...
    router.push(`/live-action${queryString ? `?${queryString}` : ''}`);
  };

  return (
    <div className={styles.content}>
      <div className={styles.headerSection}>
        <h1 className={styles.pageTitle}>Live Action & Asian Dramas</h1>
        <p className={styles.pageSubtitle}>Stream live-action adaptations, J-Dramas, K-Dramas, and Asian cinema.</p>
      ...
      {shows.length === 0 && !loading ? (
        <div style={{ textAlign: 'center', padding: '4rem 1rem', color: '#aaa' }}>
          <p style={{ fontSize: '1.1rem', marginBottom: '1rem' }}>No live-action titles found.</p>
        </div>
      ) : (
        <>
...
export default function LiveActionPage() {
  return (
    <main className={styles.container}>
      <Suspense fallback={<div className={styles.loading}>Loading Live Action...</div>}>
        <LiveActionContent />
      </Suspense>
      <Footer />
    </main>
